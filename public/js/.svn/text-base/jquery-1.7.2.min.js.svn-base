...
//登录检测
var UserSign=false;
if($("#head .topmymsg b").html()!=""){
    UserSign=true;
}

//弹出层显示、关闭效果。用法：alertShow(cs,ch) cs为点击显示的元素；ch为点击隐藏的元素 如 alertShow("#a","#b")
function alertShow(cs,ch,wrap){
    cs.click(function(){
        if($.browser.msie) {
            wrap.show();
        }else{
            wrap.show().animate({"top":"100px","opacity":"1"},400);
        }
    });
    ch.click(function(){
        if($.browser.msie) {
            ...
            wrap.animate({"top":"0px","opacity":"0"},400,function(){
                wrap.hide();
            })
        }
    });
...
$(function(){
    //新建直播弹出层
    $("#directwrap").css({"left":(parseInt($(window).width())-parseInt($("#directbg").children().width()))/2+"px"});
    $("#directbg").css({"height":$("#directbg").children().height()});
    $("#livelogin").css({"left":(parseInt($(window).width())-parseInt($("#directbg").children().width()))/2+"px"});
    $("#liveloginbg").css({"height":$("#directbg").children().height()});
    $("#addlive").css({"left":(parseInt($(window).width())-parseInt($("#directbg").children().width()))/2+"px"});
    $("#addlivebg").css({"height":$("#addlivebg").children().height()});
    $("#deliver").css({"left":(parseInt($(window).width())-parseInt($("#directbg").children().width()))/2+"px"});
    $("#deliverbg").css({"height":$("#directbg").children().height()});
    //图片追加
    $(".dimg").each(function(index){
        if($(this).attr("img")=="http://zhibo.yesky286x215.com" || $(this).attr("img")=="undefined" ){
            return
        }else{
    ...
    //火狐兼容性
    if ($.browser.mozilla) {
        $("#wrap").css({"position":"relative","margin-top":"-20px"})
    }
});