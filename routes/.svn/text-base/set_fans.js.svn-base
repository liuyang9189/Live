...
function dopage(/*totalNum,curPage*/){
    ...
    for(var i=0;i<totalNum;i++){
        pageHtml+="<li page='"+(i+1)+"'><a href='http://127.0.0.1:8080/set_fans/"+(i+1)+"' target='_self'>"+(i+1)+"</a></li>";
    }
    return pageHtml;
}
function nextpage(curPage){
    if(curPage<totalNum){
        var nextHtml="<li style='width:65px'><a style='width:65px' href='http://127.0.0.1:8080/set_fans/"+(parseInt(curPage)+1)+"' target='_self'>下一页</a></li>"
    }
    return nextHtml;
}
function prevpage(curPage){
    if(curPage!=1){
        var prevHtml="<li style='width:65px'><a style='width:65px' href='http://127.0.0.1:8080/set_fans/"+(parseInt(curPage)-1)+"' target='_self'>上一页</a></li>"
    }
    return prevHtml;
}

exports.set_fans = function(req, res){
    ...
    dbpool.execute(function(db){
        ...
        wrap.setAllDoneCallback(function(results){
            ...
            if(user){
                res.render("set_fans",results);
            }
        });
        db.collection('relations', function (err, coll) {
            if(pagenumber==1){pagenumber = 0}else{pagenumber = parseInt(pagenumber-1)};
            var limit=7;
            coll.find({relate_id:user._id}).toArray(wrap.wrap(function(err,directs){
                totalNum = Math.ceil(directs.length/limit);
            }));
            coll.find({relate_id:user._id},{skip:pagenumber*limit,limit:limit}).toArray(wrap.wrap(function(err,dt){
                for(var i=0;i<dt.length;i++){
                    (function(n){
                        var oId = dt[n].user_id;
                                ...
                                db.collection('direct', function (err, conn) {
                                    conn.find({pin:oPin}).toArray(wrap.wrap(function(err,datas){
                                        for(var l=0;l<datas.length;l++){
                                            fansPraise=fansPraise+datas[l].praise;
                                        }
                                        dt[n].praise=fansPraise;
                                        fansPraise=null;
                                    }));
                                });
                            }));
